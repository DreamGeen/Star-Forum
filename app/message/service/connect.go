...
func Run(c *gin.Context) {
	...
	if err != nil {
		utils.Logger.Error("upgrader connection failed", zap.Error(err))
	...
	if err != nil {
		utils.Logger.Error("get user id failed", zap.Error(err))
		str.Response(c, err, str.Empty, nil)
		return
	}
	instanceId, err := getHostnameInstanceID()
	if err != nil {
		utils.Logger.Error("get hostname instance id failed", zap.Error(err))
		str.Response(c, str.ErrServiceBusy, str.Empty, nil)
		return
	}
	if err := SaveServiceId(userId, instanceId); err != nil {
		utils.Logger.Error("save service id failed", zap.Error(err))
		str.Response(c, err, str.Empty, nil)
		return
	}
