...
func UpdatePostComment(tx *sqlx.Tx, postId, count int64) error {
	...
	if err != nil {
		logger.CommentLogger.Error("更新帖子评论数失败", zap.Error(err), zap.Int64("postId", postId))
		return err
	}
	return nil
}
...
func UpdateReplyComment(tx *sqlx.Tx, commentId, count int64) error {
	...
	if err != nil {
		logger.CommentLogger.Error("更新评论回复数失败", zap.Error(err), zap.Int64("commentId", commentId))
		return err
	}
	return nil
}
...
func CreateComment(comment *models.Comment) error {
	logger.CommentLogger.Info("开始发布评论")
	// 开始事务
	tx, err := db.Beginx()
	if err != nil {
	...
	defer func() {
		if err != nil {
			...
			if err = tx.Rollback(); err != nil {
				logger.CommentLogger.Error("发布评论回滚事务失败", zap.Error(err))
			}
		} else {
			// 如果没有错误，则提交事务
			if err = tx.Commit(); err != nil {
				logger.CommentLogger.Error("发布评论事务提交失败", zap.Error(err))
			}
		}
	}()
	...
	if comment.BeCommentId != 0 {
		...
		if err := CheckComment(comment.BeCommentId); err != nil {
			...
			if err != nil {
				logger.CommentLogger.Error("评论发布:检查父评论的帖子ID失败", zap.Error(err))
				return err
			}
			if parentPostId != comment.PostId {
				logger.CommentLogger.Error("评论发布:父评论的帖子ID与当前评论的帖子ID不一致", zap.Int64("parentPostId", parentPostId), zap.Int64("commentPostId", comment.PostId))
	...
	if err != nil {
		logger.CommentLogger.Error("评论发布:评论插入数据库失败", zap.Error(err))
		return err
	}
	...
	if err != nil {
		logger.CommentLogger.Error("评论发布:更新帖子评论数失败", zap.Error(err))
		return err
	}
	...
	if comment.BeCommentId != 0 {
		...
		if err != nil {
			logger.CommentLogger.Error("更新父评论回复数失败", zap.Error(err))
			return err
		}
	}

	logger.CommentLogger.Info("评论发布成功", zap.Int64("commentId", commentId))
...
func DeleteComment(commentId int64) error {
	logger.CommentLogger.Info("开始删除评论")
	// 开始事务
	tx, err := db.Beginx()
	if err != nil {
		logger.CommentLogger.Error("开启事务失败", zap.Error(err))
		return err
	}
	defer func() {
		if err != nil {
			// 如果发生错误，则回滚事务
			if err = tx.Rollback(); err != nil {
				logger.CommentLogger.Error("删除评论事务回滚失败", zap.Error(err))
			}
		} else {
			// 如果没有错误，则提交事务
			if err = tx.Commit(); err != nil {
				logger.CommentLogger.Error("删除评论事务提交失败", zap.Error(err))
			}
		}
	}()
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			// 评论不存在或已被删除
			logger.CommentLogger.Error("尝试删除的评论ID不存在或已被删除", zap.Int64("commentId", commentId))
			return fmt.Errorf("评论ID: %d 不存在或已被删除", commentId)
		}
		logger.CommentLogger.Error("评论删除:查询评论关联的postId失败", zap.Error(err))
		return err
	}

	logger.CommentLogger.Info("准备递归删除评论")

	// 递归删除评论
	if err := deleteComments(tx, commentId, postId); err != nil {
		logger.CommentLogger.Error("递归删除评论失败", zap.Error(err))
		return err
	}

	logger.CommentLogger.Info("评论ID: %d 删除成功", zap.Int64("commentId", commentId))
	return nil
}
...
func deleteComments(tx *sqlx.Tx, commentId int64, postId int64) error {
	...
	defer func() {
		if err := rows.Close(); err != nil {
			logger.CommentLogger.Error("递归删除评论:关闭rows失败", zap.Error(err))
		}
	}()
...
func GetCommentsStar(postId int64) ([]*models.Comment, error) {
	...
	defer func() {
		if err := rows.Close(); err != nil {
			logger.CommentLogger.Error("评论获取:关闭rows时出错", zap.Error(err))
		}
	}()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&comment.CommentId, &comment.PostId, &comment.UserId, &comment.Content, &comment.Star, &comment.Reply, &comment.BeCommentId, &comment.CreatedAt); err != nil {
			// 如果扫描失败，返回错误
			logger.CommentLogger.Error("评论获取:调用Scan错误", zap.Error(err))
	...
	if err := rows.Err(); err != nil {
		// 如果有错误，返回错误
		logger.CommentLogger.Error("评论获取:迭代出错", zap.Error(err))
	...
	logger.CommentLogger.Info("评论获取成功", zap.Int64("postId", postId))
	return comments, nil
}

// UpdateStar 点赞评论
func UpdateStar(commentId int64, increment int8) error {
	logger.CommentLogger.Info("开始点赞评论")
	_, err := db.Exec(starComment, increment, commentId)
	if err != nil {
		logger.CommentLogger.Error("更新点赞数时出错", zap.Error(err))
		return err
	}

	logger.CommentLogger.Info("评论点赞成功", zap.Int64("commentId", commentId))
	return nil
}
