...
func main() {
	...
	tp, err := tracing.SetTraceProvider(str.AdminService)
	if err != nil {
	...
	microService := micro.NewService(
		micro.Name(str.AdminService), //服务名称
		micro.Version("v1"),          //服务版本
		micro.Registry(etcdReg),      //etcd注册件
	)
	//服务注册
	if err := adminPb.RegisterAdminServiceHandler(microService.Server(), adminIns); err != nil {
		panic(err)
	}
	//服务启动
