...
func main() {
	...
	defer CloseMQ()
	tp, err := tracing.SetTraceProvider(str.LikeService)
	if err != nil {
	...
	microService := micro.NewService(
		micro.Name(str.LikeService), //服务名称
		micro.Version("v1"),         //服务版本
		micro.Registry(etcdReg),     //etcd注册件
	)
	//服务注册
	if err := likePb.RegisterLikeServiceHandler(microService.Server(), likeSrvIns); err != nil {
		panic(err)
	}
	//服务启动
